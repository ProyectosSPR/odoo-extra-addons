...
publicWidget.registry.dateRange = publicWidget.Widget.extend({
    selector: '.widget_daterange',
    events: {
    ...
    start() {
        ...
        this.$inputDateRange.attr('readonly', true);
    ...
    dr_as_string() {
        ...
        let dr = data.day_start.real_day.toString() + ' ' + MONTHS_SHORT[data.day_start.real_month - 1] + ' - ' +
    ...
    onSelectDateRange (event) {
        ...
        if (data.day_start && !data.day_end) {
            const new_day = { month: month,
                              day: day,
                              real_year: real_year,
                              real_month: real_month,
                              real_day: real_day }
            if (((month == data.day_start.month) && (day > data.day_start.day)) || (month > data.day_start.month)) {
                data.day_end = new_day
            } else {
                data.day_start = new_day
            }
        } else {
            data.day_end = false;
            data.day_start = {
